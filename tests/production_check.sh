...
# T2: today（未認証・初回）→ total 0・DB 自動作成
body=$(curl -sk --max-time 20 "$BASE/api.php?action=today")
if printf '%s' "$body" | grep -q '"total":0'; then ok "T2 today → total 0"; else ng "T2 today" "$body"; fi
...
# T4: today → total 2
body=$(curl -sk --max-time 20 "$BASE/api.php?action=today")
if printf '%s' "$body" | grep -q '"total":2'; then ok "T4 today → total 2"; else ng "T4 today" "$body"; fi
...
# T6: delete（認証済み）→ 204・today が 0 に復元
code=$(curl -sk --max-time 20 -b "$JAR" -X DELETE -o /dev/null -w '%{http_code}' "$BASE/api.php?action=delete&id=$ID")
body=$(curl -sk --max-time 20 "$BASE/api.php?action=today")
if [ "$code" = "204" ] && printf '%s' "$body" | grep -q '"total":0'; then ok "T6 delete → 204・total 0 復元"; else ng "T6 delete" "code=$code today=$body"; fi
...
# T10: PHP 環境（プローブ・検証後は削除）
pr=$(curl -sk --max-time 20 "$BASE/probe.php")
if printf '%s' "$pr" | grep -q 'PHP=8' && printf '%s' "$pr" | grep -q 'pdo_sqlite=yes'; then ok "T10 PHP 8.x + pdo_sqlite ($pr)"; else ng "T10 probe" "$pr"; fi
