...
※本番デプロイ後のブラウザ UI 検証（本番 URL をヘッドレス chrome で操作: カレンダー閲覧・admin PW ログイン・2026-06 集計・日詳細・月報/分析グラフ描画）: 本番オリジン直接実行は **3 回とも実行中にアクセス元 IP が一時アクセス規制（全ドメイン 403・coreserver の IP 単位規制）に掛かり中断**（P1・P2 = 本番がアプリを正常ロード・表示することはブラウザ実測で確認済み・規制はアプリ/デプロイ起因でない）。**配置済みアーティファクト（SFTP mirror・`diff -r` で本番と byte 一致）+ 本番 DB スナップショット（401件）をローカル配信して同一ハーネスを実行 → 16/16 PASS**（reports/2026-08-07-production-browser-ui.txt）。本番の HTTP 経路は受入 17/17 が検証済み。
